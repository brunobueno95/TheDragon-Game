...
<img src="imgs/icon-materials.png" style="height:35px;width:35px;margin-left:17px;"> Materials: ${model.PlayerItems.yourMaterials.length}</div><hr>
    ...
    width: 98.5vw;
    height: 80vh;
    background-image:    url(imgs/desertFind.jpg);
    ...
    if(something.picture.includes("creature") ){
       let html=""
        ...
        html     += `
            ...
            <div>
            <button style="width:200px;height:40px; font-size: 15px; padding-bottom: 12px;" onclick="collectMaterialDs()">Check finding</button>
        </div>
        ` 
        console.log(html)
        // model.forestFindHtml = html
    ...
    if(something.picture.includes("gold") ){
        ...
        <div class="random-find">
           <div>
            ...
            <div>
            <button style="width:200px;height:40px;font-size: 15px; padding-bottom: 12px;" onclick="collectCoinsDs()">Check finding</button>
            </div>
        </div>
        `
        `
            ...
            <div>
            <button onclick="collectCoinsDs()">Check finding</button>
        </div>
        ` 
         console.log(html)
        //  model.forestFindHtml = html
